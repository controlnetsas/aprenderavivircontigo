...
//
// Este archivo centraliza los formularios del sitio que piden nombre/correo y aún no tienen
// backend propio (Test de Genealogía, Test del Eneatipo y las Heridas, Test Regresando al
// Vientre) y los conecta a dos servicios externos GRATUITOS, pensados para sitios estáticos:
//
...
//      a) "Bienvenida al Club" — sin uso actualmente (la membresía ahora la activa membresia.php
//         en el servidor). Se deja disponible por si se necesita en el futuro.
//      b) "Confirmación" — plantilla compartida para el Test de Genealogía, el Test del Eneatipo y
//         las Heridas, y el Test Regresando al Vientre. Variables: {{nombre}}, {{correo}}, {{motivo}}.
//    Copia el Service ID, los dos Template ID y el Public Key (Account → General) y pégalos abajo.
//
// Nota: el formulario de Contacto (contacto.html) YA NO usa este archivo — ese registro lo procesa
// el servidor directamente vía procesar-inscripcion-ajax.php (ver assets/js/main.js), guardando en
// la base de datos real y sincronizando con HubSpot y Brevo del lado del servidor.
//
...
(function () {
  ...
  window.AVC_REGISTRO = {
    ...
    guardar: function (nombre, correo, whatsapp, origen) {
      this.datos = { nombre: nombre, correo: correo, whatsapp: whatsapp || '', origen: origen || 'genealogia-test' };
      this.enviarACRM();
    },
    ...
    // Sin uso actualmente (queda disponible por si se necesita en el futuro).
    ...
    // Genealogía, el Test del Eneatipo y las Heridas, y el Test Regresando al Vientre.
